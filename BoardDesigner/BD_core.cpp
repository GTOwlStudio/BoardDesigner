...
BD_core::BD_core(bool enableValidation) : core_base(enableValidation), gui(mousePos)
{
	/*width = 1280;
	height = 720;*/
	zoom = -2.5f;
...
void BD_core::setImageLayout(VkCommandBuffer cmdBuffer, VkImage image, VkImageAspectFlags ascpectMask, VkImageLayout oldImageLayout, VkImageLayout newImageLayout, uint32_t mipLevel, uint32_t mipLevelCount)
{
...
void BD_core::generateCoord(float x, float y, float w, float h, float depth)
{
#define DIM 1.0f
...
void BD_core::setupVertexDescription()
{
...
void BD_core::prepareUniformBuffers()
{
...
void BD_core::updateUniformBuffers()
{
...
void BD_core::setupDescriptorSetLayout()
{
...
void BD_core::setupDescriptorPool()
{
...
void BD_core::setupDescriptorSet()
{
...
void BD_core::loadTexture(std::string filename, VkFormat format, bool forceLinearTiling)
{
	gli::texture2D tex2D(gli::load(filename));
		...
		if (useStaging) {
			...
			VkCommandBuffer copyCmd = core_base::createCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
			...
			core_base::flushCommandBuffer(copyCmd, queue, true);
	...
	void BD_core::destroyTextureImage(Texture texture) {
		vkDestroyImage(device, texture.image, nullptr);
		vkFreeMemory(device, texture.deviceMemory, nullptr);
	}

	void BD_core::buildCommandBuffers() {
