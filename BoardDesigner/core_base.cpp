...
core_base::core_base(bool enableValidation)
{
#if defined(_WIN32)
...
void core_base::initVulkan(bool enableValidation)
{
	VkResult err;
...
void core_base::setupConsole(std::string title)
{
	AllocConsole();
...
HWND core_base::setupWindow(HINSTANCE hinstance, WNDPROC wndproc)
{
...
void core_base::handleMessages(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
	case WM_CLOSE:
...
void core_base::viewChanged()
{
}

void core_base::keyPressed(uint32_t keyCode)
{
}

void core_base::windowResized()
{
}

VkBool32 core_base::getMemoryType(uint32_t typeBits, VkFlags properties, uint32_t * typeIndex)
{
...
uint32_t core_base::getMemoryType(uint32_t typeBits, VkFlags properties)
{
	for (uint32_t i = 0; i < 32; i++) {
		if ((typeBits & 1) == 1) {
			if ((deviceMemoryProperties.memoryTypes[i].propertyFlags & properties) == properties) {
...
void core_base::createCommandPool()
{
...
void core_base::setupDepthStencil()
{
...
void core_base::setupFrameBuffer()
{
...
void core_base::setupRenderPass()
{
...
void core_base::setupSwapchain()
{
	swapChain.create(setupCmdBuffer, &width, &height);
}

void core_base::createCommandBuffers()
{
...
void core_base::destroyCommandBuffers()
{
...
void core_base::createSetupCommandBuffer()
{
	if (setupCmdBuffer != VK_NULL_HANDLE) {
		vkFreeCommandBuffers(device, cmdPool, 1, &setupCmdBuffer);
...
void core_base::createPipelineCache()
{
...
void core_base::flushSetupCommandBuffer()
{
	VkResult err;
...
VkCommandBuffer core_base::createCommandBuffer(VkCommandBufferLevel level, bool begin)
{
...
void core_base::flushCommandBuffer(VkCommandBuffer commandBuffer, VkQueue queue, bool free)
{
...
VkPipelineShaderStageCreateInfo core_base::loadShader(std::string fileName, VkShaderStageFlagBits stage)
{
...
VkBool32 core_base::createBuffer(VkBufferUsageFlags usageFlags, VkMemoryPropertyFlags memoryPropertyFlags, VkDeviceSize size, void * data, VkBuffer * buffer, VkDeviceMemory * memory)
{
	VkMemoryRequirements memReqs;
...
VkBool32 core_base::createBuffer(VkBufferUsageFlags usage, VkDeviceSize size, void * data, VkBuffer * buffer, VkDeviceMemory * memory)
{
	return createBuffer(usage, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, size, data, buffer, memory);
}

VkBool32 core_base::createBuffer(VkBufferUsageFlags usage, VkDeviceSize size, void * data, VkBuffer * buffer, VkDeviceMemory * memory, VkDescriptorBufferInfo * descriptor)
{
...
void core_base::loadMesh(std::string filename, vkMeshLoader::MeshBuffer * meshBuffer, std::vector<vkMeshLoader::VertexLayout> vertexLayout, float scale)
{
...
void core_base::submitPrePresentBarrier(VkImage image)
{
	VkCommandBufferBeginInfo cmdBufInfo = vkTools::initializers::commandBufferBeginInfo();
...
void core_base::submitPostPresentBarrier(VkImage image)
{
	VkCommandBufferBeginInfo cmdBufInfo = vkTools::initializers::commandBufferBeginInfo();
...
const std::string core_base::getAssetPath()
{
	return "./data/";
}

VkResult core_base::createInstance(bool enableValidation)
{
...
VkResult core_base::createDevice(VkDeviceQueueCreateInfo requestedQueues, bool enableValidation)
{
...
std::string core_base::getWindowTitle()
{
